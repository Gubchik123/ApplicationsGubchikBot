...
@dp.message(
    lambda message: users.get(message.from_user.id, {}).get("status") == "admin"
...
@dp.message(
    lambda message: user_state.get(message.from_user.id) == "waiting_for_user_id"
)
...
@dp.message(
    lambda message: user_state.get(message.from_user.id) == "waiting_for_new_status"
)
...
async def list_domains(message: Message):
    ...
    if not user_domains:
        ...
        await message.answer(
            "Виберіть домен, який хочете видалити:", reply_markup=domain_keyboard
        )
...
async def back_to_main_menu(message: Message):
    user_id = message.from_user.id
    user_state[user_id] = (
        "main_menu"  # Скидаємо стан користувача або встановлюємо на основний стан
    )
    await message.answer(
...
# Waiting fot domain
@dp.message(
    lambda message: user_state.get(message.from_user.id) == "waiting_for_domain"
)
...
@dp.message(
    lambda message: (
        user_state.get(message.from_user.id) == "waiting_for_start"
        or user_state.get(message.from_user.id) == "main_menu"
    )
...
@dp.message(
    lambda message: user_state.get(message.from_user.id)
    in ["waiting_for_frequency", "waiting_for_duration"]
    and message.text
    in [
        "Без затримки 🚀",
...
async def handle_frequency_and_duration(message: Message):
    ...
    if user_state[user_id] == "waiting_for_frequency":
        ...
        if user_data.get("status") == "demo":
            ...
            asyncio.create_task(
                request_loop(user_id, frequency, duration=None)
    ...
    if user_state[user_id] == "waiting_for_duration":
        ...
        asyncio.create_task(
            request_loop(user_id, frequency, duration=user_durations.get(user_id))
        )
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                async with session.get(
                    user_urls[user_id], proxy=proxy_url, headers=headers
                ) as response:
...
async def request_loop(user_id, frequency, duration):
    ...
    while user_state.get(user_id) == "active" and requests_to_send > 0:
        ...
        if error_message:
            ...
            await bot.send_message(
                user_id, "⬇️ Використовуйте кнопку нижче:", reply_markup=start_keyboard
            )
            break
    ...
    if user_state.get(user_id) == "active":
        user_state[user_id] = "waiting_for_start"
        users[user_id]["applications_sent"] += user_request_counter[
            user_id
...
# Обробник зупинки
@dp.message(
    lambda message: user_state.get(message.from_user.id) == "active"
